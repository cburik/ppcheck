...
class CsvExtractor:
    def __init__(self, path: str, column_order: List[str] = ["account_name", "username", "url", "password"]):
    ...
    def extract(self) -> List[Account]:
        ...
        self.accounts = [
            Account.create_account(row.account_name, row.username, row.url, row.password)
